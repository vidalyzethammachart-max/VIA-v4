...
BEGIN
  ...
  v_title := COALESCE(NULLIF(trim(COALESCE(p_case_title, '')), ''), v_case_key);
  ...
  IF NOT FOUND THEN
    INSERT INTO public.video_cases (
      case_key, title, case_title, order_number, subject_name, short_code,
      source_file_name, video_object_key, created_by
    ) VALUES (
      v_case_key, v_title, v_title,
  ...
  ELSIF (
    ...
    OR EXISTS (
      SELECT 1 FROM public.video_case_members vcm
      WHERE vcm.video_case_id = v_case.id AND vcm.user_id = v_actor
    )
  ) THEN
  ...
  IF p_member_role = 'leader' AND EXISTS (
    SELECT 1 FROM public.video_case_members vcm
  ...
  INSERT INTO public.video_case_members (video_case_id, user_id, member_role)
  VALUES (v_case.id, v_actor, v_role)
